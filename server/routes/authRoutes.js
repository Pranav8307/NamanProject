...
router.get('/google', oauthRateLimit, (req, res) => {
  try {
    ...
    res.redirect(`${process.env.CLIENT_URL || 'http://localhost:5174'}/login?error=oauth_init_failed`);
  }
});
...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    if (oauthError) {
      ...
      return res.redirect(`${process.env.CLIENT_URL || 'http://localhost:5174'}/login?error=${oauthError}`);
    }
    ...
    if (!code || !state) {
      ...
      return res.redirect(`${process.env.CLIENT_URL || 'http://localhost:5174'}/login?error=missing_params`);
    }
    ...
    if (!storedData || storedData.provider !== 'google') {
      ...
      return res.redirect(`${process.env.CLIENT_URL || 'http://localhost:5174'}/login?error=invalid_state`);
    }
    ...
    if (user && user.isNonceUsed(storedData.nonce)) {
      ...
      return res.redirect(`${process.env.CLIENT_URL || 'http://localhost:5174'}/login?error=security_error`);
    }
    ...
    if (!user) {
      ...
      if (!user.isActive) {
        ...
        return res.redirect(`${process.env.CLIENT_URL || 'http://localhost:5174'}/login?error=account_inactive`);
      }
    ...
    res.redirect(`${process.env.CLIENT_URL || 'http://localhost:5174'}${returnTo}`);
  } catch (error) {
    ...
    res.redirect(`${process.env.CLIENT_URL || 'http://localhost:5174'}/login?error=oauth_failed`);
  }
});
...
router.post('/facebook/data-deletion', async (req, res) => {
  try {
    ...
    res.json({
      url: `${process.env.CLIENT_URL || 'http://localhost:5174'}/data-deletion-status?confirmation=${confirmationCode}`,
