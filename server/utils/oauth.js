...
const getFacebookRedirectUri = () => {
  return process.env.FACEBOOK_REDIRECT_URI || 'http://localhost:5001/auth/facebook/callback';
};
...
const getFacebookTokens = async (code, codeVerifier) => {
  ...
  const params = {
    ...
    redirect_uri: process.env.FACEBOOK_REDIRECT_URI || 'http://localhost:5001/auth/facebook/callback',
    code: code,
    code_verifier: codeVerifier
  };
